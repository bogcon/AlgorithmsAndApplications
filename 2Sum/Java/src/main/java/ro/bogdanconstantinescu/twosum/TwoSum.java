...
public class TwoSum {
	...
	/**
	 * The sum to find 2 elements for that summed equals it.
	 */
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < vector.length; i++) {
			if (vector[i] != sum - vector[i]
					&& -1 != BinarySearch.binarySearch(vector, 0,
							vector.length - 1, sum - vector[i])) {
				found = true;
				x = vector[i];
				y = sum - vector[i];
				break;
	...
	public static Long[] readVectorFromFile(String file) throws Exception {
		...
		try {
			...
			if (sc.hasNextInt()) {
				...
				throw new Exception(
						"Could not read the number of elements vector has.");
			}
			if (sc.hasNextLong()) {
				sum = sc.nextLong();
			} else {
				throw new Exception(
						"Could not read target sum to search for if there are 2 values x + y = sum.");
			}
			v = new Long[n];
			for (int i = 0; i < n; i++) {
				if (sc.hasNextLong()) {
