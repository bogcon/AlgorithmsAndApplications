package ro.bogdanconstantinescu.medianmaintenance;

/*
 * Median Maintenance problem. This particular implementation prints the sum of
 * medians.
 *
 * @author      Bogdan Constantinescu <bog_con@yahoo.com>
 * @since       2013.07.08
 * @version     1.0
 * @link        GitHub  https://github.com/bogcon/AlgorithmsAndApplications/tree/master/MedianMaintenance
...
public class MedianMaintenance {
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length == 0) {
				throw new Exception(
						"The input file must be given as an argument.");
			}
	 ...
	 *            The vector to print.
	 */
	public static void printVector(final int[] v) {
	...
	public static int[] readVectorFromFile(final String file) throws Exception {
		...
		try {
			...
			if (!sc.hasNextInt()) {
				throw new Exception(
						"Could not read the number of elements the vector has.");
			}
			n = sc.nextInt();
			v = new int[n];
			for (int i = 0; i < n; i++) {
