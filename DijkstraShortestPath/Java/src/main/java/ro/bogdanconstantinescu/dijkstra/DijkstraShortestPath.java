...
public class DijkstraShortestPath {
	...
	public static void main(String[] args) throws Exception {
		...
		/* print result */
		for (int i = 0; i < graph.n; i++) {
			System.out.println("Shortest path from " + (startVertex + 1) + " to " + (i + 1)
					+ " is: " + shortestPaths[i]);
		}
		System.out.println();
		System.out
				.println("Elapsed: " + readTime + " seconds with initializations, reading graph.");
	 ...
	 *
	 * @param file
	 *            The file where to read the graph from.
	 * @return The read graph.
	 * @throws Exception
	 */
	public static Graph readGraphFromFile(final String file) throws Exception {
		Scanner sc;
			...
			while (sc.hasNextLine()) {
				...
				while (st.hasMoreElements()) {
					...
					if (st2.hasMoreElements()) {
						...
						throw new Exception("Could not read weight between " + vertex + " and "
								+ neighbor);
					}
