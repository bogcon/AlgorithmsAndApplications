...
public class MinHeap {
	 ...
	 *            Heap 's dimension.
	 */
	public MinHeap(final int heapDimension) {
		this.lastFreePos = 0;
	 ...
	 *             If heap is full.
	 */
	public void insert(final HeapNode hn) throws Exception {
	...
	public HeapNode extractMin() throws Exception {
		...
		// It 's basically a deletion from the first position without needing to
		// bubble up
		...
		while ((leftChildPos < this.lastFreePos && this.heapNodes[parentPos]
				.compareTo(this.heapNodes[leftChildPos]) > 0)
				|| (rightChildPos < this.lastFreePos && this.heapNodes[parentPos]
						.compareTo(this.heapNodes[rightChildPos]) > 0)) {
			minChildPos = leftChildPos;
			if (rightChildPos < this.lastFreePos
					&& this.heapNodes[leftChildPos].compareTo(this.heapNodes[rightChildPos]) > 0) {
	 ...
	 *
	 * @param pos
	 *            The element 's position in heap.
	 * @return The deleted element.
	 * @throws Exception
	 *             If position is out of permitted bounds.
	 */
	public HeapNode delete(final int pos) throws Exception {
		...
		if (parentPos >= 0 && null != this.heapNodes[childPos]
				&& this.heapNodes[parentPos].compareTo(this.heapNodes[childPos]) > 0) { // bubble
																						// up
			while (parentPos >= 0
					&& this.heapNodes[parentPos].compareTo(this.heapNodes[childPos]) > 0) {
				aux = this.heapNodes[parentPos];
				this.heapNodes[parentPos] = this.heapNodes[childPos];
				this.heapNodes[childPos] = aux;
				this.positions[this.heapNodes[childPos].vertex] = childPos;
