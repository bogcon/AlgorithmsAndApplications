package ro.bogdanconstantinescu.inversionscount;
 ...
 * @since       2013.07.05
 * @version     1.0
 * @link        GitHub  https://github.com/bogcon/AlgorithmsAndApplications/tree/master/InversionsCount
...
public class InversionsCountTest {
	...
	/**
	 * Test case for {@link InversionsCount#merge(int[], int[])}.
	 */
	@Test
	public void mergeTest() {
		final int[] merged1 = InversionsCount.merge(this.mergeV11, this.mergeV12);
		assertEquals("merged 1 length is not ok", this.mergeV11.length + this.mergeV12.length,
				merged1.length);

		final int[] merged2 = InversionsCount.merge(this.mergeV12, this.mergeV11);
		...
		final int[] merged3 = InversionsCount.merge(this.mergeV21, this.mergeV22);
		assertEquals("merged 3 length is not ok", this.mergeV21.length + this.mergeV22.length,
				merged3.length);

		final int[] merged4 = InversionsCount.merge(this.mergeV22, this.mergeV21);
		...
		final int[] merged5 = InversionsCount.merge(this.mergeV11, this.mergeV22);
		assertEquals("merged 5 length is not ok", this.mergeV11.length + this.mergeV22.length,
				merged5.length);

		final int[] merged6 = InversionsCount.merge(this.mergeV22, this.mergeV11);
	...
	/**
	 * Test case for {@link InversionsCount#mergeSort(int[])}.
	 */
	@Test
	...
	/**
	 * Test case for {@link InversionsCount#inversionsCount}.
	 */
	@Test
